    ...
    companion object {
        ...
        suspend fun fetchNextEvents(number: Long): MutableList<Event> {
            val querySnapshot: QuerySnapshot =
                if (offset == null) {
                    Firebase.firestore.collection(EVENTS).orderBy("eventID").limit(number).get()
                        .await()
                } else {
                    Firebase.firestore
                        .collection(EVENTS)
        ...
        private fun mapDocToEvent(document: DocumentSnapshot): Event? {
            ...
            return Event(
                ...
                // TODO: Add organizer
                organizer = Profile.fromUID(Firebase.auth.currentUser!!.uid)
            )
        }
                    ...
                    "inscriptionLimitDate" to
                            when (event.inscriptionLimitDate) {
                                null -> "null"
                                    ...
                                    event.inscriptionLimitDate.format(
                                        DateTimeFormatter.ofPattern(
                                            DATE_FORMAT
                                        )
                                    )
                    ...
                    "inscriptionLimitTime" to
                            when (event.inscriptionLimitTime) {
                                null -> "null"
                                    ...
                                    event.inscriptionLimitTime.format(
                                        DateTimeFormatter.ofPattern(
                                            TIME_FORMAT
                                        )
                                    )
        ...
        fun deleteEvent(eventID: String) {
            Firebase.firestore.collection(EVENTS).document(eventID).delete()
                .addOnFailureListener { exception ->
