...
class RatingFirebaseConnection {
  ...
  fun deleteEventRating(eventID: String) {
    ...
    runBlocking { async { deleteAttendeesRating(eventID) }.await() }
    ...
    aggregateQuery
        ...
        .addOnSuccessListener { result ->
          ...
          try {
            ...
            Log.e(TAG, "Error formatting average rating average is $avg " + "count is $count ", e)
            return@addOnSuccessListener
          }
  ...
  */
   ...
   */
  fun aggregateOrganizerRatings(organizerID: String) {
    var ratings: Map<String, List<Pair<String, Any>>>? = null
    runBlocking {
      ratings = async { fetchOrganizerRatings(organizerID) }.await() ?: return@runBlocking
    }
    ...
    Firebase.firestore
        ...
        .addOnSuccessListener {
          Log.d(
              TAG,
              "Organizer $organizerID updated with average rating $avg and rating count $nRatings")
        }
  }
