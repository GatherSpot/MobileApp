...
class ChatFirebaseConnection : FirebaseConnectionInterface {
    ...
    override fun getFromDocument(d: DocumentSnapshot): CollectionClass? {
        ...
        val messages = (d.get("messages") as List<*>).map { (it as HashMap<*, *>)}
            .map{ Message(it["id"] as String, it["senderID"] as String, it["content"] as String, it["timestamp"] as Timestamp, it["read"] as Boolean)}
